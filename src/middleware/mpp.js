/**
 * HiveLens — MPP (Machine Payments Protocol) Middleware
 *
 ...
 *
 * Adapted from _canonical/mpp.js — service identity changed to hivelens.
 */
...
function getLensPrice(path) {
  if (LENS_PRICING[path] !== undefined) return LENS_PRICING[path];
  if (path.startsWith('/lens/agent/')) return 0.05;
  return 0.10; // default
}

// ─── Free paths ──────────────────────────────────────────────

// Under app.use('/v1', ...) req.path has /v1 stripped.
 ...
 *   3. No Payment header → pass (x402 handles 402 challenge)
 */
async function mppMiddleware(req, res, next) {
  ...
  const expectedAmount = getLensPrice(req.path);
